...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Get the pacman position and the jail position
        ppos = gameState.getPacmanPosition()
        jpos = self.getJailPosition()

        # Update the beliefs based on the positions
        for gpos in self.allPositions:
            self.beliefs[gpos] *= self.getObservationProb(observation, ppos, gpos, jpos)
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            ...
            for p in newPosDist:
                newProb[p] += newPosDist[p] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for p in range(self.numParticles):
            pos = self.legalPositions[p % size]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # Get each particles observation probability
        ...
        # Re intialize if total weight is 0 else sample
        if totalSum == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [tmp.sample() for x in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        # sample each particle's next state based on its current state
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        # shuffle the list of permutations in order to ensure even placement of particles
        random.shuffle(p)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            trivial = 1 # trivial number since we are multiplying

            # loop over all the ghosts
            for i in range(self.numGhosts):
                trivial *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), p[i], self.getJailPosition(i))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i, p in enumerate(newParticle):
