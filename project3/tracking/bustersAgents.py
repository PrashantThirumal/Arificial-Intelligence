...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Initialize a min distance val and min action val
        minDist = float('inf')
        minAct = None

        # Iterate through ghost positions and find the closest one
        for beliefs in livingGhostPositionDistributions:
            distance = self.distancer.getDistance(beliefs.argMax(), pacmanPosition)
            if distance < minDist:
                minDist = distance
                minPos = beliefs.argMax()

        # Reset min distance val and then find the minimum action for the closest ghost
        minDist = float('inf')

        # Iterate through the possible actions and find the smallest one
        for a in legal:
            nextLoc = Actions.getSuccessor(pacmanPosition, a)
            distance = self.distancer.getDistance(minPos, nextLoc)
            if distance < minDist:
                minDist = distance
                minAct = a
